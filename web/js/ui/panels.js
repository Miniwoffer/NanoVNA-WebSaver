...
export function devicePanel(state) {
  const portSelect = el('select.input');
  ...
  const refresh = async () => {
    ...
    clear(portSelect);
    if (!ports.length) {
      portSelect.append(el('option', { value: '', textContent: 'No granted ports' }));
    }
    ports.forEach((entry, index) => {
      portSelect.append(el('option', { value: String(index), textContent: entry.label }));
    });
  };

  const connect = guard(state, async () => {
    const index = Number(portSelect.value);
    const entry = ports[index];
    if (!entry) throw new Error('Choose a port, or add one with "Add device"');
    await state.connect(entry.port);
  });
  ...
  const addDevice = guard(state, async () => {
    ...
    if (!port) return;
    await refresh();
    const index = ports.findIndex((entry) => entry.port === port);
  ...
  const body = el(
    'div',
    {},
    field('Port', portSelect),
    el(
      'div.row',
      {},
      button('Add device', addDevice, { variant: 'primary' }),
      button('Connect', connect),
      button('Disconnect', guard(state, () => state.disconnect())),
    ),
    info,
  );

  const node = panel('Device', body);

  if (!serialSupported()) {
    body.prepend(el('p.warning', {}, serialUnsupportedReason()));
  }
  ...
  const renderInfo = () => {
    ...
    if (!device) {
      info.append(el('p.muted', {}, 'Not connected.'));
      return;
    }
    const rows = [
      ['Model', device.name],
    ...
    if (device.features.has('Bandwidth')) {
      ...
      device
        ...
        .catch(() => {});
      info.append(field('Bandwidth', bandwidthSelect));
    }
